...
=== TEST 4: Forward client request with proxy
--- http_config
    lua_package_path "/usr/local/lib/lua/?.lua;/usr/local/openresty/lualib/?.lua;;";
    lua_ssl_trusted_certificate "/etc/ssl/certs/ca-certificates.crt";
    lua_ssl_verify_depth 3;
...
--- config
    ...
    location  = /vRfnOj4y/init.js {
        ...
        set_by_lua_block $config {
            ...
            pxconfig.first_party_enabled = true
            pxconfig.px_appId = "PXvRfnOj4y"
            pxconfig.proxy_url = 'http://10.128.0.24:3128'
            return true
        }
...
=== TEST 5: Forward XHR requests with proxy
...
=== TEST 6: Forward captcha script request with proxy
